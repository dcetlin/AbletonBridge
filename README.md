...
AbletonBridge gives Claude (or any MCP client) direct control over Ableton Live sessions — 355 MCP tools backed by 262 handler commands via a `@command` registry, three transport layers (TCP, UDP/OSC, HTTP), TypedDict codegen, and structured error responses.
...
- **355 MCP tools** across 17 modules — session, tracks, clips, mixer, devices, browser, automation, arrangement, scenes, creative generation, M4L deep access, snapshots, audio, grid notation, MIDI CC, LOM, and compound workflows
